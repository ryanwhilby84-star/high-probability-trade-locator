...
function App(){
  ...
  React.useEffect(()=>{fetch('/data/confluence_history_latest.json').then(r=>r.json()).then(j=>{setData(j.records||[]); const ds=[...new Set((j.records||[]).map(r=>r.cot_report_date))].sort(); setDate(ds.at(-1)||'')})},[])
  const dates=React.useMemo(()=>[...new Set(data.map(r=>r.cot_report_date))].sort(),[data])
  const week=React.useMemo(()=>data.filter(r=>r.cot_report_date===date).map(r=>({...r,market_key:canonical(r.market)})),[data,date])
  React.useEffect(()=>{if(!market && week[0]) setMarket(week[0].market)},[week,market])
  const filtered=week.filter(r=>(!search||r.market.toLowerCase().includes(search.toLowerCase())) && (bias==='All'||(r.confluence_bias||'').includes(bias.replace(' Bias',''))) && (readiness==='All'||r.trade_readiness===readiness))
  const missing=EXPECTED.filter(e=>!week.some(w=>canonical(w.market)===e))
  const counts=(k)=>Object.entries(week.reduce((a,r)=>(a[r[k]]=(a[r[k]]||0)+1,a),{})).map(([name,value])=>({name,value}))
  const series=data.filter(r=>r.market===market).sort((a,b)=>a.cot_report_date.localeCompare(b.cot_report_date))
  ...
  <div className='charts'>
    <Chart title='Confluence Score by Market'><BarChart data={week}><CartesianGrid strokeDasharray='3 3'/><XAxis dataKey='market'/><YAxis/><Tooltip/><Bar dataKey='confluence_score' fill='#4fd1c5'/></BarChart></Chart>
