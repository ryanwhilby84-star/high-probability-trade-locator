"""CLI: Natural Gas Valuation — Driver Validation Phase 3 (LNG Exports).

Research-only. Does not modify published fair value or weekly COT.

Usage:
  python scripts/run_ng_driver_validation_phase3_lng.py
"""
...
from hptl.valuation.ng_driver_validation_phase3_lng import (  # noqa: E402
    run_phase3_lng_validation,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Phase 3: prove whether LNG exports deserve promotion from "
    ...
    payload = run_phase3_lng_validation(as_of_week=args.as_of_week)
    paths = write_phase3_outputs(payload)
    elapsed = round(time.perf_counter() - t0, 2)

    print("NG Driver Validation Phase 3 (LNG)")
    ...
    print(
        f"  dataset: {ds.get('series_id')} n={(ds.get('history_available') or {}).get('n_observations')} "
        f"tip={ds.get('current_observation_date')}"
    )
    ...
    print(f"  LNG_RECOMMENDATION: {payload.get('lng_recommendation')}")
    print(f"  {payload.get('plain_english')}")
    print(f"  published_model={payload.get('published_model_id')} unchanged={payload.get('published_model_unchanged')}")
