"""CLI: Natural Gas Valuation — Macro Validation Phase 4 (Broad US Dollar).

Research-only. Does not modify published fair value or weekly COT.

Usage:
  python scripts/run_ng_driver_validation_phase4_broad_dollar.py
"""
...
from hptl.valuation.ng_driver_validation_phase4_broad_dollar import (  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Phase 4: prove whether Broad US Dollar deserves promotion from "
    ...
    payload = run_phase4_broad_dollar_validation(as_of_week=args.as_of_week)
    paths = write_phase4_outputs(payload)
    elapsed = round(time.perf_counter() - t0, 2)

    print("NG Macro Validation Phase 4 (Broad US Dollar)")
    ...
    print(f"  BROAD_DOLLAR_RECOMMENDATION: {payload.get('broad_dollar_recommendation')}")
