...
class UnifiedPriceAdapter:
    ...
    def fetch(self, instrument_id: str, *, spec: InstrumentSpec | None = None) -> InstrumentPriceRecord:
        ...
        if not source and instrument_id in FRED_COT_FAIL_SERIES: source="fred"
        ...
        oanda_sym=OANDA_STORE_SYMBOL.get(instrument_id) or (oanda_symbol_for(spec,self._coverage) if source=="oanda" else None)
        # One OANDA attempt only.  Preserve the actual provider error instead of
        # swallowing it and making a duplicate request that loses diagnostics.
        if source=="oanda":
