...
Research-only unless Real Yield clears every promotion gate.
Does not modify weekly COT / HPTL_SKIP_VALUATION / published valuation.

Compares:
  A) Storage-only
  B) Storage + Production YoY  (current published v2)
  C) Storage + Production YoY + Real Yield transform  (one transform at a time)
"""
...
AUDIT_DIR = PROJECT_ROOT / "data" / "audits" / "ng_driver_validation_phase5_real_yield"
JSON_OUT = AUDIT_DIR / "phase5_real_yield_validation.json"
MD_OUT = AUDIT_DIR / "phase5_real_yield_validation.md"

SERIES_ID = "DFII10"
SERIES_LABEL = "Market Yield on U.S. Treasury Securities at 10-Year Constant Maturity, Quoted on an Investment Basis, Inflation-Indexed"

# Higher real yields → higher opportunity cost / risk-off pressure on commodities →
# negative coef on log(NG price).
TRANSFORM_SPECS: list[tuple[str, str, str]] = [
    ("raw_yield", "raw 10Y real yield level (%, as-of weekly)", "negative"),
    ("yoy_chg", "year-over-year change in real yield (percentage points)", "negative"),
    ("chg_4w", "4-week change in real yield (pp)", "negative"),
    ("chg_12w", "12-week change in real yield (pp)", "negative"),
    ("rolling_zscore_156", "trailing 156-week z-score (past-only mean/sd)", "negative"),
    (
        "trend_deviation_260",
        "deviation from trailing 260-week (~5y) linear trend (past-only)",
        "negative",
    ),
]


def document_real_yield_dataset() -> dict[str, Any]:
    cfg = _load_config()
    fred_map = cfg.get("fred_series") or {}
    # Prefer explicit real-yield key if present; else metals-style DFII10.
    series_id = str(
        fred_map.get("real_yield_10y")
        or fred_map.get("us_real_yield_10y")
        or SERIES_ID
    )
    ...
    return {
        ...
        "release_cadence": (
            "Daily Treasury constant-maturity TIPS yield; typically available "
            "same day or next business day via FRED."
        ),
        "history_available": {
        ...
        "point_in_time_safety": {
            ...
            "alignment": (
                "Daily real-yield levels as-of joined to Friday NG weeks "
                "(last observation on or before week date). Transforms use only "
                "past weekly as-of levels — no full-sample z-score. "
                "YoY uses absolute pp change (safe when yields cross zero)."
            ),
        },
        "revisions_policy": (
            "Treasury constant-maturity yields via FRED are market quotes; "
            "material delayed revision risk is low. Research treats downloaded "
            "daily values as point-in-time as-of."
        ),
        "alignment_with_ng_history": {
            "ng_panel_uses_weekly_asof": True,
            "sufficient_history_for_walk_forward": len(dates) >= 400,
            "note": (
                "Daily DFII10 covers the post-2016 NG weekly valuation panel. "
                "Safe to align with storage + production YoY history via as-of join."
            ),
        },
        "expected_economic_sign": "negative",
        "economic_rationale": (
            "Higher 10Y real yields raise the opportunity cost of holding "
            "commodities and often coincide with tighter financial conditions, "
            "so the coefficient on log(NG price) should be negative."
        ),
    }


def _trend_deviation(xs: list[float], window: int = 260) -> list[float | None]:
...
def _yoy_change_pp(xs: list[float]) -> list[float | None]:
    """Year-over-year absolute change in percentage points (not % of level)."""
    n = len(xs)
    out: list[float | None] = [None] * n
    for i in range(n):
        if i < 52:
            continue
        out[i] = float(xs[i]) - float(xs[i - 52])
    return out


def _build_real_yield_transforms(
    dates: list[str], level: list[float]
) -> dict[str, list[float | None]]:
    """Individual Real Yield transforms — no combinations, no full-sample leakage."""
    base = _build_production_transforms(dates, level)
    return {
        "raw_yield": list(base["raw_level"]),  # type: ignore[arg-type]
        "yoy_chg": _yoy_change_pp(level),
        "chg_4w": base["chg_4w"],
        "chg_12w": base["chg_12w"],
        "rolling_zscore_156": _rolling_z_past_only(level, 156),
        "trend_deviation_260": _trend_deviation(level, 260),
    }
...
def _promotion_decision(
    *,
    ...
    gates = {
        ...
        "real_yield_sign_ok": False,
        "real_yield_coef_stable_no_flip": False,
    ...
    ry_name = next(
        (f for f in (candidate.get("features") or []) if f.startswith("real_yield__")),
        None,
    )
    if ry_name:
        coef = (candidate.get("coefficients") or {}).get(ry_name)
        gates["real_yield_sign_ok"] = bool(coef is not None and coef < 0)
        if not gates["real_yield_sign_ok"]:
            reasons.append(
                f"Real Yield coefficient sign not economically sensible "
                f"(got {coef}; expect negative)"
            )
        stab = (candidate.get("coefficient_stability") or {}).get(ry_name) or {}
        gates["real_yield_coef_stable_no_flip"] = not bool(stab.get("sign_flip"))
        if not gates["real_yield_coef_stable_no_flip"]:
            reasons.append("walk-forward Real Yield coefficient sign flips")
    else:
        reasons.append("Real Yield feature missing")
    ...
    if promote:
        ...
        plain = (
            f"Transform '{transform_id}' clears every Real Yield promotion gate "
...
def run_phase5_real_yield_validation(*, as_of_week: str | None = None) -> dict[str, Any]:
    generated_at = datetime.now(timezone.utc).replace(microsecond=0).isoformat()
    dataset = document_real_yield_dataset()
    ...
    if not level_ok or len(level) != bundle.n:
        return {
            "generated_at": generated_at,
            "ok": False,
            "error": "Could not as-of align Real Yield to full NG weekly panel",
            "real_yield_dataset": dataset,
        }
    ...
    ry_transforms = _build_real_yield_transforms(bundle.dates, level)
    ...
    for transform_id, label, exp_sign in TRANSFORM_SPECS:
        series = ry_transforms[transform_id]
        dates, y, s, p, ry = _align_triple(
        ...
        ry_feat = f"real_yield__{transform_id}"
        candidate = _eval_model(
            name=f"C_storage_prod_yoy_ry_{transform_id}",
            dates=dates,
            y=y,
            feature_names=["storage_surplus_bcf", "production_yoy_pct", ry_feat],
            cols=[s, p, ry],
            expected_signs={
                "storage_surplus_bcf": "negative",
                "production_yoy_pct": "negative",
        ...
        dm["interprets"] = (
            "Positive mean_loss_diff means Storage+ProdYoY+RealYield has lower MSE than v2."
        )
        ...
        n = len(y)
        while t < n:
            beta, r2 = _multivariate_ols(y[:t], [s[:t], p[:t], ry[:t]])
        ...
        candidates.append(
            {
                ...
                "candidate_storage_prod_yoy_real_yield": slim_c,
    ...
    if promoted:
        ...
        overall = "Promote"
        overall_plain = (
            f"Promote Real Yield into fair value only as `{best.get('transform_id')}`. "
        ...
        if ok_cands and reject_count == len(ok_cands):
            overall = "Reject"
            overall_plain = (
                "Reject Real Yield for fair value. No transform improves "
            ...
            overall_plain = (
                "Real Yield remains Experimental. No transform clears all "
    ...
    if overall == "Keep Experimental":
        ...
        overall_plain = (
            "Reject Real Yield for promotion into the published fair-value model. "
            + overall_plain
        )
    ...
    payload = {
        "generated_at": generated_at,
        "ok": True,
        "phase": "ng_driver_validation_phase5_real_yield",
        "scope": {
            "candidate_driver": "US 10-Year Real Yield",
            "series_id": series_id,
            ...
            "not_tested": [
                ...
                "Liquidity",
                "Broad USD (already rejected Phase 4)",
            ...
            "promotion_thresholds": {
                ...
                "expected_real_yield_sign": "negative",
                "no_sign_flip": True,
                "no_leakage": True,
        ...
        "best_candidate": (
            {
                ...
                "candidate_metrics": {
                    "oos_rmse": (
                        best.get("candidate_storage_prod_yoy_real_yield") or {}
                    ).get("oos_rmse"),
                    "oos_mae": (
                        best.get("candidate_storage_prod_yoy_real_yield") or {}
                    ).get("oos_mae"),
        ...
        "real_yield_recommendation": published_decision,
        ...
        "proposed_model_id_if_promoted": "ng_storage_production_real_yield_v3",
        "note": (
            "Research-only. Weekly COT untouched. No nominal yields / inflation "
            "expectations / liquidity tested."
        ),
    }
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    for t in payload.get("real_yield_transforms_tested") or []:
        lines.append(
            f"- `{t.get('id')}` — {t.get('label')} (expect {t.get('expected_sign')})"
        )
    lines += [
        "",
    ...
    for c in payload.get("candidates") or []:
        ...
        ry_key = next(
            (k for k in (sp.get("coefficients") or {}) if k.startswith("real_yield__")),
            None,
        )
        coef = (sp.get("coefficients") or {}).get(ry_key) if ry_key else None
        stab = (
            ((sp.get("coefficient_stability") or {}).get(ry_key) or {}) if ry_key else {}
        )
    ...
    lines += [
        ...
        "",
        (
            "Economically, higher 10Y real yields should pressure NG prices "
        ...
        "## Recommendation",
        "",
        f"**{payload.get('real_yield_recommendation')}** "
...
def write_phase5_outputs(payload: dict[str, Any]) -> dict[str, Path]:
