"""Download CFTC commodity + financial futures reports and export workbook + processed CSV."""
...
class WorkbookExportPaths:
    workbook_path: object  # Path from ExportResult.export_file_path
    processed_csv_path: object
...
def run_workbook_export() -> WorkbookExportPaths:
    """Download latest disaggregated + financial futures ZIPs, merge, export workbook and ``cot_cleaned_*.csv``."""
    setup_logging()
    settings = get_settings()
    warnings: list[str] = []

    log_step("Workbook export: downloading CFTC commodity ZIP…")
    download = download_latest_cot(settings)
    log_kv("commodity download path", download.raw_file_path.resolve())
    ...
    log_step("Workbook export: downloading CFTC financial futures ZIP…")
    financial_download = download_financial_futures_only_history(settings, year=settings.cot_year)
    log_kv("financial download path", financial_download.raw_file_path.resolve())
    ...
    def _fmt_max(rows: pd.DataFrame) -> str | None:
        m = _max_report_date(rows)
        return m.strftime("%Y-%m-%d") if m is not None else None

    log_kv("parsed max report date (commodity)", _fmt_max(commodity_dashboard_rows))
    log_kv("parsed max report date (financial index)", _fmt_max(index_history_df))
    log_kv("parsed max report date (combined)", _fmt_max(combined_dashboard_rows))
    ...
    log_kv("processed CSV path", export.processed_csv_path.resolve())
    log_kv("workbook path", export.export_file_path.resolve())
    logger.info("COT workbook export complete: %s", export.export_file_path)
    return WorkbookExportPaths(
