"""Location pillar: Bullish / Neutral / Bearish from 52-week price percentile and range position.

This measures *where price is* in its recent history — not fundamental fair value.
"""
...
def _location_state_from_percentile(pct: float) -> str:
    if pct <= 33.0:
        return LOCATION_LOW
    if pct >= 67.0:
...
def _score_from_percentile(pct: float) -> float:
    """0–10 conviction from distance from range midpoint (50th pct)."""
...
def compute_location(
    *,
    ...
    """Returns location_bias, location_score (0–10), location_reason, price_percentile_52w."""
    ...
    if pct is None:
        return {
            ...
            "wired": False,
            "location_bias": "UNAVAILABLE",
            "location_state": "UNAVAILABLE",
            "location_score": None,
            "location_reason": "Insufficient weekly price history for 52-week location.",
            "price_percentile_52w": None,
            "pass": False,
        }

    bias = _bias_from_percentile(pct)
    state = _location_state_from_percentile(pct)
    ...
    if macro_note == "weak_macro_link":
        ...
        reason += " Macro driver correlation noted (location only — not valuation)."
...
def location_pass(bias: str, direction: str) -> bool:
    if bias == "UNAVAILABLE":
        return False
    d = direction.lower()
    if d == "long":
